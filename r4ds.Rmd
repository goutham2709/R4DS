...
```
geom_bar makes height of the bar proportional to the number of cases (count) in each group.
geom_col represents the height of the raw data. geom_bar creates two variables, count and prop.
...
follow http://sape.inf.usi.ch/quick-reference/ggplot2/geom

4. What variables does stat_smooth() compute? What parameters control its behaviour?
```{r}
?stat_smooth
```
See computed variabels in documentation
y = predicted value
ymin = lower confidence interval around the mean
ymax = upper confidence interval around the mean
se = standard error
Method, formula, na.rm contorl it's behaviour.
...
?stat_count
```
..prop.. computes groupwise proportion. If group = 1 argument is not given, the geom_bar plots all the bars with the same height. In the two plots, the proportions are calculated with the groups.
...
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
  geom_jitter()
```
We can see by plotting both the graphs above. geom jitter adds random noise and makes the graph much more scattered than the geom_point, where all the points appear to be stacked on top of each other. It is a useful way of handling overplotting caused by discreteness in small datasets.
...
```
width, height and na.rm control the amount of jittering. The default jittering is 40% of the resolution of the data.
...
```
geom_count is a variation of the geom_point, that counts the number of observations at each location and sizes the points relative to the counts. Useful when we have discrete data and overplotting. geom_jitter also handles overplotting caused by discrete data. But it handles this overplotting by adding small amount of random variation to the location of each point.
...
```
dodge2 is the default position adjustment for geom_boxplot.
Available position adjustments:
...
?labs
```
modifies title, subtiitle, caption, tags and label of the plots.
...
ggplot(nz, aes(long, lat, group = group)) +
  ...
  coord_quickmap()
```
coord_map Maps projections and these projections in genereal doesnot preserve straight lines. coord_quickmap is a quick approximation that preserves the straight lines and takes into account the curvature of the earth by using latitudes and longitudes.
...
?geom_abline
```
geom_abline adds a reference line. coord_fixed makes the ratio of units on the both the axes equal to 1. It is basically used to manipulate the aspect ratio of the plots.
